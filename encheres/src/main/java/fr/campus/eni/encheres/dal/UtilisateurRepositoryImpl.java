...
""";
  ...
  public List<Utilisateur> getAll() {
    String sql =
        "select no_utilisateur as noUtilisateur, pseudo, nom, prenom, email, telephone, rue,"
            + " code_postal as codePostal, ville, mot_de_passe as motDePasse, credit,"
            + " administrateur from utilisateurs";
    List<Utilisateur> utilisateurs =
        namedParameterJdbcTemplate.query(sql, new BeanPropertyRowMapper<>(Utilisateur.class));
  ...
  public Optional<Utilisateur> getById(int id) {
    String sql =
        "select no_utilisateur as noUtilisateur, pseudo, nom, prenom, email, telephone, rue,"
            + " code_postal as codePostal, ville, mot_de_passe as motDePasse, credit,"
            + " administrateur from utilisateurs where no_utilisateur = ?";
    Utilisateur utilisateur = null;
    try {
      utilisateur =
          jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(Utilisateur.class), id);
  ...
  public Optional<Utilisateur> getByPseudo(String pseudo) {
    ...
    try {
      utilisateur =
          jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(Utilisateur.class), pseudo);
    } catch (DataAccessException exc) {
      exc.printStackTrace();
